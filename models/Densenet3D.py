...
class SinglePathDenseNet(BaseModel):
    def __init__(
        ...
        self.features.add_module(
            "conv1x1_1",
            nn.Conv3d(
                total_conv_channels, 400, kernel_size=1, stride=1, padding=0, bias=False
            ),
        )
        ...
        self.features.add_module(
            "conv1x1_2",
            nn.Conv3d(400, 200, kernel_size=1, stride=1, padding=0, bias=False),
        )
        ...
        self.features.add_module(
            "conv1x1_3",
            nn.Conv3d(200, 150, kernel_size=1, stride=1, padding=0, bias=False),
        )
        ...
        self.classifier.add_module(
            "classifier",
            nn.Conv3d(
                150, self.num_classes, kernel_size=1, stride=1, padding=0, bias=False
            ),
        )
...
class DualPathDenseNet(BaseModel):
    def __init__(self, in_channels, classes=4, drop_rate=0, fusion="concat"):
        ...
        if self.input_channels == 2:
            ...
            self.stream_2 = SinglePathDenseNet(
                in_channels=1,
                drop_rate=drop_rate,
                classes=classes,
                return_logits=False,
        ...
        if self.input_channels == 3:
            self.stream_1 = SinglePathDenseNet(
                in_channels=1, drop_rate=drop_rate, classes=classes, return_logits=False
            )
            self.stream_2 = SinglePathDenseNet(
                in_channels=1, drop_rate=drop_rate, classes=classes, return_logits=False
            )
            self.stream_3 = SinglePathDenseNet(
                in_channels=1, drop_rate=drop_rate, classes=classes, return_logits=False
            )
    ...
    def forward(self, multi_channel_medical_img):
        ...
        if channels != self.input_channels:
            ...
            if self.input_channels == 2:
                ...
                if self.fusion == "concat":
                    concat_features = torch.cat(
                        (output_features_t1, output_features_t2, output_features_t3),
                        dim=1,
                    )
                    return self.classifier(concat_features)
                else:
...
class DualSingleDenseNet(BaseModel):
    ...
    def __init__(
        ...
        if self.input_channels == 2:
            self.early_conv_1 = _HyperDenseLayer(
                num_input_features=1, num_output_channels=25, drop_rate=drop_rate
            )
            self.early_conv_2 = _HyperDenseLayer(
                num_input_features=1, num_output_channels=25, drop_rate=drop_rate
            )
            single_path_channels = 52
            self.stream_1 = SinglePathDenseNet(
                in_channels=single_path_channels,
                drop_rate=drop_rate,
                classes=classes,
                return_logits=True,
                early_fusion=True,
        ...
        if self.input_channels == 3:
            self.early_conv_1 = _HyperDenseLayer(
                num_input_features=1, num_output_channels=25, drop_rate=0
            )
            self.early_conv_2 = _HyperDenseLayer(
                num_input_features=1, num_output_channels=25, drop_rate=0
            )
            self.early_conv_3 = _HyperDenseLayer(
                num_input_features=1, num_output_channels=25, drop_rate=0
            )
