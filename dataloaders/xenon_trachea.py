...
class XenonTrachea(tio.SubjectsDataset):
    def __init__(
        self,
    ...
    def get_subjects_list(mode: str, dataset_path: str):
        ...
        for root, dirs, files in os.walk(mode_path):
            if any("mask_trachea" in file for file in files):
                gas_path = os.path.join(root, "gas.nii")
                mask_path = os.path.join(root, "mask_trachea.nii")
        ...
        if augmentation:
            transforms.extend(
                [
                    ...
                    # tio.RandomElasticDeformation(max_displacement=4, p=0.5),
