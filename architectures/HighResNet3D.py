...
class ConvInit(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Conv3d(
            self.in_channels, self.num_features, kernel_size=3, padding=1
        )
...
class DilatedConv2(nn.Module):
    def __init__(self, in_channels):
        ...
        relu1 = nn.ReLU()
        conv1 = nn.Conv3d(
            self.in_channels, self.num_features, kernel_size=3, padding=2, dilation=2
        )
...
class DilatedConv4(nn.Module):
    def __init__(self, in_channels):
        ...
        relu1 = nn.ReLU()
        conv1 = nn.Conv3d(
            self.in_channels, self.num_features, kernel_size=3, padding=4, dilation=4
        )
...
class HighResNet3D(BaseModel):
    def __init__(self, in_channels=1, classes=4, shortcut_type="A", dropout_layer=True):
        ...
        if dropout_layer:
            conv_out = nn.Conv3d(
                self.dil4_channels, self.conv_out_channels, kernel_size=1
            )
            drop3d = nn.Dropout3d()
