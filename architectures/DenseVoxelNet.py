...
class _Transition(nn.Module):
    def __init__(self, num_input_features, num_output_features):
        ...
        conv3d = nn.Conv3d(
            num_input_features, num_output_features, kernel_size=1, padding=0, stride=1
        )
...
class DenseVoxelNet(BaseModel):
    ...
    def __init__(self, in_channels=1, classes=3):
        ...
        self.conv_final = nn.Conv3d(
            self.up_out_features, classes, kernel_size=1, padding=0, bias=False
        )
        self.transpose = nn.ConvTranspose3d(
    ...
    def test(self, device="cpu"):
        ...
        summary(
            self.to(torch.device(device)), (self.in_channels, 8, 8, 8), device=device
        )
        b, c = self.forward(a)
