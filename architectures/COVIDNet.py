...
class CovidNet(nn.Module):
    def __init__(self, model="large", n_classes=3):
        ...
        if model == "large":
            self.add_module(
                "conv1_1x1", nn.Conv2d(in_channels=64, out_channels=256, kernel_size=1)
            )
            self.add_module(
    ...
    def forward_large_net(self, x):
        ...
        pepx36 = self.pepx3_6(
            pepx31 + pepx32 + pepx33 + pepx34 + pepx35 + out_conv3_1x1
        )

        out_conv4_1x1 = F.max_pool2d(
            self.conv4_1x1(
                pepx31 + pepx32 + pepx33 + pepx34 + pepx35 + pepx36 + out_conv3_1x1
            ),
            2,
        )
