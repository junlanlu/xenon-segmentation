...
class BaseModel(nn.Module, ABC):
    ...
    def __init__(self):
        ...
        Attributes:
            best_loss (float): The best loss value so far. Used for checkpointing.
                Defaults to 1000000.
        """
    ...
    @abstractmethod
    def forward(self, x):
        """Forward pass of the model to be implemented by the subclass."""
        pass

    @abstractmethod
    def test(self):
        """To be implemented by the subclass so that models can perform a forward
        propagation
        """
        pass
    ...
    def restore_checkpoint(
        self, ckpt_file: str, optimizer: Optional[torch.optim.Optimizer] = None
    ):
    ...
    def save_checkpoint(
        self, directory: str, epoch: int, loss, optimizer=None, name=None
    ):
        ...
        if name is None:
            name = "{}_{}_epoch.pth".format(
                os.path.basename(directory), "last"  # netD or netG
            )
